export const NAV_LINKS = [
  { label: "Work", href: "#work" },
  { label: "Services", href: "#services" },
  { label: "Pricing", href: "#pricing" },
  { label: "FAQ", href: "#faq" },
];

export const AGENCY_NAME = "wedesignslides";

export const PRICING_PLANS = [
  {
    title: "Student",
    price: "$29",
    description: "Perfect for assignments & projects.",
    features: ["Up to 10 Slides", "Topic Research", "24h Delivery", "Source File Included", "1 Revision"],
    highlight: false,
  },
  {
    title: "Business",
    price: "$49",
    description: "Professional reports & internal decks.",
    features: ["Up to 20 Slides", "Topic Research", "Custom Graphics", "Source File Included", "Unlimited Revisions"],
    highlight: true,
  },
  {
    title: "Pitch Deck",
    price: "$79",
    description: "Investment-ready startup presentations.",
    features: ["Up to 15 Slides", "Deep Market Research", "Investor Focused", "Unlimited Revisions"],
    highlight: false,
  },
];


